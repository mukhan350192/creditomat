...
class MfoController extends Controller
{
    ...
    public function mfoClick(Request $request)
    {
        ...
        do {
            if (!$click_id) {
                $result['message'] = 'Не передан клик айди';
                break;
            }
            if (!$mfo_id) {
                $result['message'] = 'Не передан МФО айди';
                break;
            }
            DB::beginTransaction();
            $id = DB::table('mfo_click')->insertGetId([
                'click_id' => $click_id,
                'mfo_id' => $mfo_id,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
